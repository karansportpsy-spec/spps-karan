...
import { fetchCaseDailyLogSummary } from '@/services/caseFormulationApi'
import { setAthletePortalActivation } from '@/services/athletesApi'
import {
...
export default function CaseFormulationPage() {
  ...
  const { data: athletes = [], refetch: refetchAthletes } = useAthletes()
  ...
  const { data: dailySummaryPayload, refetch: refetchDailySummary } = useQuery({
    queryKey: ['case_daily_summary', athleteId],
    enabled: !!athleteId && !!user,
    queryFn: async () => fetchCaseDailyLogSummary(athleteId!),
  })
  ...
  async function handlePortalActivationToggle() {
    ...
    try {
      const currentlyActivated = Boolean((athlete as any).is_portal_activated)
      const nextActivated = !currentlyActivated
      const response = await setAthletePortalActivation(athlete.id, nextActivated, nextActivated)

      queryClient.setQueryData(['athletes', user?.id], (existing: any) => {
        if (!Array.isArray(existing)) return existing
        return existing.map((row: any) =>
          row.id === athlete.id
            ? {
                ...row,
                is_portal_activated: response.athlete?.is_portal_activated,
                portal_activated_at: response.athlete?.portal_activated_at,
              }
            : row
        )
      })
                ...
                <div className="grid sm:grid-cols-2 gap-2 mt-3">
                  <div className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Athlete Portal</p>
                      <span
                        className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${
                          (athlete as any).is_portal_activated
                            ? 'bg-green-100 text-green-700'
                            : 'bg-gray-200 text-gray-600'
                        }`}
                      >
                        {(athlete as any).is_portal_activated ? 'Activated' : 'Deactivated'}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                      {(athlete as any).is_portal_activated && (athlete as any).portal_activated_at
                        ? `Activated on ${fmtDate((athlete as any).portal_activated_at)}`
                        : 'Athlete login is blocked until activated.'}
                    </p>
                    <button
                      type="button"
                      onClick={handlePortalActivationToggle}
                      disabled={portalUpdating}
                      className="mt-2 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-blue-200 text-blue-700 bg-blue-50 hover:bg-blue-100 disabled:opacity-60"
                    >
                      {portalUpdating
                        ? 'Updating...'
                        : (athlete as any).is_portal_activated
                  ...
                  <div className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2">
                    <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Daily Reflection Feed</p>
                    <p className="text-xs text-gray-500 mt-1">
                      {dailySummary?.total_logs
                        ? `${dailySummary.total_logs} total logs · Avg mood ${dailySummary.avg_mood ?? 0} · Avg stress ${dailySummary.avg_stress ?? 0}`
                        : 'No daily logs available yet.'}
                    </p>
                    {dailySummary?.recent_logs?.find((entry: any) => entry.reflection)?.reflection ? (
                      <p className="text-xs text-gray-600 mt-1.5 line-clamp-2 italic">
                        "
                        {
                          dailySummary.recent_logs.find((entry: any) => entry.reflection)
                            ?.reflection
                        }
                        "
                      </p>
