  ...
  return useMutation({
    mutationFn: async (payload: any) => {
      ...
      const missingColumnRegex =
        /Could not find the ['"]([^'"]+)['"] column|column ["']([^"']+)["'] of relation ["']psychophysiology["'] does not exist/i
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        if (error.code === '23502' && msg.includes('record_type')) {
          row.record_type = row.record_type || 'manual'
          continue
        }
      ...
      throw new Error('Failed to save psychophysiology record after compatibility retries.')
    },
