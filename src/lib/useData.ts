...
// All core data hooks — Sessions, Check-ins, Assessments, Interventions
// Follows the same pattern as useAthletes.ts
...
export function useSessions(athleteId?: string) {
  ...
  return useQuery<Session[]>({
    ...
    queryFn: async () => {
      let q = supabase
        .from('sessions')
        .select('*, athlete:athletes(id,first_name,last_name,sport,uid_code)')
...
export function useCreateSession() {
  ...
  return useMutation({
    mutationFn: async (payload: Omit<Session, 'id' | 'practitioner_id' | 'created_at' | 'updated_at' | 'athlete'>) => {
...
export function useCreateCheckIn() {
  ...
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (payload: Omit<CheckIn, 'id' | 'practitioner_id' | 'created_at' | 'athlete'>) => {
      const { data, error } = await supabase
        .from('check_ins')
        .insert({ ...payload, practitioner_id: user!.id })
        .select()
...
export function useCreateAssessment() {
  ...
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (payload: Omit<Assessment, 'id' | 'practitioner_id' | 'created_at' | 'athlete'>) => {
      const { data, error } = await supabase
        .from('assessments')
        .insert({ ...payload, practitioner_id: user!.id })
        .select()
...
export function useCreateIntervention() {
  ...
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (payload: Omit<Intervention, 'id' | 'practitioner_id' | 'created_at' | 'updated_at' | 'athlete'>) => {
      const { data, error } = await supabase
        .from('interventions')
        .insert({ ...payload, practitioner_id: user!.id })
        .select()
