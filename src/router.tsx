...
//   • Removed the dual-auth path (RequireAthletePortalAuth / RedirectIfAthletePortalAuth)
//     that depended on the legacy custom-token backend. Athletes now authenticate
//     entirely through Supabase Auth — matching the AcceptInvitePage flow.
//   • `/athlete/portal` now redirects to `/athlete/dashboard` for backwards
//     compatibility with older invite links that pointed there.
//   • `/athlete/login` is guarded by `RedirectIfAthleteAuth` which reads the
//     Supabase session.
//
...
function RedirectIfAthleteAuth({ children }: { children: React.ReactNode }) {
  const { user, loading } = useAuth()
  if (loading) return <LoadingScreen />
...
const router = createBrowserRouter([
  ...
  // Athlete authenticated surface (Supabase session with role='athlete')
  {
